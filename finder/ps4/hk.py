...
class PS4Hk(Store):
    ...
    def __init__(self):
        ...
        self.count_url = "https://store.playstation.com/valkyrie-api/ch/HK/999/container/STORE-MSF86012-GAMESALL?size=0&bucket=games&start=0"
    ...
    def getData(self, size=1, page=1):
        url = "https://store.playstation.com/valkyrie-api/ch/HK/999/container/STORE-MSF86012-GAMESALL?size=%d&bucket=games&start=%d"
        offset = (page - 1) * size
        url = url % (size, offset)
        print(url)
        ...
        for data in data_list["included"]:
            ...
            if data["type"] == "game":
                ...
                game.publishDate = int(time.mktime(time.strptime(info["release-date"], "%Y-%m-%dT%H:%M:%SZ"))) if info["release-date"] else 0
